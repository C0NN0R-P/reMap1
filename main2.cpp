...
int main(int argc, char *argv[])
{
    ...
    while(usedAddresses.size() < numAddressTotal)
    {
        ...
        long long channelCounts[4] = {0};
        long long rankCounts[8] = {0};
        long long bankCounts[16] = {0};
        long long bankGroupCounts[4] = {0};
        ...
        for(unsigned int channel = 0; channel < 4 && !found; channel++)
        {
            for(unsigned int rank = 0; rank < 8 && !found; rank++)
            {
                ...
                for(unsigned int bankGroup = 0; bankGroup < 4; bankGroup++)
                {
                    ...
                    access(adr,numAccess);
                    auto count = stopMeasure(fd);
                    bankGroupCounts[bankGroup] += count;
                }
            }
        }
        ...
        cout << "Channels:\n";
        for(int i = 0; i < 4; i++)
            cout << "Channel " << i << ": " << channelCounts[i] << " accesses\n";
        cout << "\nRanks:\n";
        for(int i = 0; i < 8; i++)
            cout << "Rank " << i << ": " << rankCounts[i] << " accesses\n";
        cout << "\nBanks:\n";
        for(int i = 0; i < 16; i++)
            cout << "Bank " << i << ": " << bankCounts[i] << " accesses\n";
        cout << "\nBank Groups:\n";
        for(int i = 0; i < 4; i++)
            cout << "BankGroup " << i << ": " << bankGroupCounts[i] << " accesses\n"; 
