...
void startMeasure(int fd)
{
    ...
    cout << "DEBUG: fd = " << fd << ", Before reset: " << count << endl;
    int reset_status = ioctl(fd, PERF_EVENT_IOC_RESET, 0);
    read(fd, &count, sizeof(long long));
    cout << "DEBUG: Reset status: " << reset_status << " (errno: " << strerror(errno) << ")" << endl;
    ioctl(fd, PERF_EVENT_IOC_ENABLE, 0);
    read(fd, &count, sizeof(long long));
    cout << "DEBUG: After reset: " << count << endl;
}

long long stopMeasure(int fd)
{
    if (fd == -1) return -1; // ensure valid fd

    long long count;
    _mm_mfence();
    ioctl(fd, PERF_EVENT_IOC_DISABLE, 0);
    read(fd, &count, sizeof(long long));
    //close(fd); DO NOT CLOSE to allow reuse
    std::cout << "DEBUG: Final count = " << count << std::endl;
    return count;
}
