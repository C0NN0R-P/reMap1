...
static int debugfs_u64_set(void *data, u64 val)
{
    ...
    list_for_each_entry(d, &skx_edac_list, list) {
        for (i = 0; i < 24; i++) {
            ...
            pr_info(MODULE_NAME ": SAD %d range: 0x%llx - 0x%llx", i, prev_limit, limit);
            if (addr >= prev_limit && addr <= limit) {
                ...
                for (mc = 0; mc < NUM_IMC; mc++) {
                    ...
                    for (tad_index = 0; tad_index < SKX_MAX_TAD; tad_index++) {
                        ...
                        u64 tad_base = SKX_TAD_BASE(base);
                        u64 tad_limit = SKX_TAD_LIMIT(wayness);
                        pr_info(MODULE_NAME ": IMC %d TAD %d: base=0x%llx limit=0x%llx", mc, tad_index, tad_base, tad_limit);
                        if (addr >= tad_base && addr <= tad_limit) {
                            ...
                            for (ch = 0; ch < NUM_CHANNELS; ch++) {
                                ...
                                for (rir_index = 0; rir_index < SKX_MAX_RIR; rir_index++) {
                                    ...
                                    u64 rir_limit = SKX_RIR_LIMIT(rirway);
                                    int ways = SKX_RIR_WAYS(rirway);
                                    pr_info(MODULE_NAME ": IMC %d CH %d RIR %d: limit=0x%llx ways=%d",
                                            mc, ch, rir_index, rir_limit, ways);
                                    if (ways == 0) {
